...
useEffect(()=>{
  const fetchCities= async()=> {
    ...
    const listA= await fetch("https://api.openaq.org/v1/latest?country="+chosenCountryCode+"&parameter=pm25&limit=10000")
        ...
        {cities.slice(0,5).map((city,index) => (
          <Card className="city-card">
            <Card.Header>
              ...
              {city.city} pm2.5 is: {city.measurements[0].value} updated at {new Date(city.measurements[0].lastUpdated).toLocaleDateString()}
      </Accordion.Toggle>
            </Card.Header>
